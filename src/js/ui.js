...
export function ui(container, page, data) {
  if (page === "main") {
    data.results.map((element) => {
      ...
      const cardDescr = el("p", {
        classes: ["card__description"],
        text: element.opening_crawl,
      });
      const cardReleaseDate = el("time", {
        classes: ["card__release-date"],
  ...
  if (page === "episodeInfo") {
    ...
    const cardInfoTitle = el("h1", {
      classes: ["card-info__title"],
      text: data.title,
    });
    const cardInfoDirector = el("h2", {
      classes: ["card-info__director"],
      text: data.director,
    });
    const cardInfoProducer = el("h3", {
      classes: ["card-info__producer"],
      text: data.producer,
    });
    const release_date = el("time", {
      classes: ["card-info__release-date"],
    ...
    if (data?.planets && data.planets.length > 0) {
      ...
      const planetsSection = el("section", {
        classes: ["section", "section-planets"],
      });
      const planetsHeader = el("div", { classes: ["section-header"] });
      const planetsTitle = el("h2", {
        classes: ["section-title", "section-title--planets"],
        text: "Планеты",
      });
      const planetsCount = el("span", {
        classes: ["section-count"],
        text: `${data.planets.length}`,
      });
      ...
      data.planets.map((element) => {
        ...
        const cardPlanetName = el("strong", {
          classes: ["card-planet__name"],
          text: element.name,
        });
        const cardPlanetDiameter = el("span", {
          classes: ["card-planet__diameter"],
          text: element.diameter,
        });
        const cardPlanetPopulation = el("span", {
          classes: ["card-planet__population"],
    ...
    if (data?.species && data.species.length > 0) {
      ...
      const speciesSection = el("section", {
        classes: ["section", "section-species"],
      });
      const speciesHeader = el("div", { classes: ["section-header"] });
      const speciesTitle = el("h2", {
        classes: ["section-title", "section-title--species"],
        text: "Виды",
      });
      const speciesCount = el("span", {
        classes: ["section-count"],
        text: `${data.species.length}`,
      });
      ...
      data.species.map((element) => {
        ...
        const cardSpecieName = el("strong", {
          classes: ["card-specie__name"],
          text: element.name,
        });
        const cardSpecieAverageHeight = el("span", {
          classes: ["card-specie__average-height"],
          text: element.average_height,
        });
        const cardSpecieClassification = el("span", {
          classes: ["card-specie__classification"],
          text: element.classification,
        });
        const cardSpecieLanguage = el("span", {
          classes: ["card-specie__language"],
          text: element.language,
        });

        cardSpecie.append(
          cardSpecieName,
          cardSpecieAverageHeight,
          cardSpecieClassification,
          cardSpecieLanguage
        );
    ...
    if (data?.characters && data.characters.length > 0) {
      ...
      const charactersSection = el("section", {
        classes: ["section", "section-characters"],
      });
      const charactersHeader = el("div", { classes: ["section-header"] });
      const charactersTitle = el("h2", {
        classes: ["section-title", "section-title--characters"],
        text: "Персонажи",
      });
      const charactersCount = el("span", {
        classes: ["section-count"],
        text: `${data.characters.length}`,
      });
      ...
      data.characters.map((element) => {
        ...
        const cardCharacterName = el("strong", {
          classes: ["card-character__name"],
          text: element.name,
        });
        const cardCharacterGender = el("span", {
          classes: ["card-character__gender"],
          text: element.gender,
        });
        const cardCharacterBirthYear = el("span", {
          classes: ["card-character__birth_year"],
          text: element.birth_year,
        });

        cardCharacter.append(
          cardCharacterName,
          cardCharacterGender,
          cardCharacterBirthYear
        );
